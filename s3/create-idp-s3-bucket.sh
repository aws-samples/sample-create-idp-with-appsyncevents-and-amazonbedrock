...
IDP_BUCKET_NAME=$(aws cloudformation describe-stacks --stack-name "$s3_bucket_stack_name" --query 'Stacks[0].Outputs[?OutputKey==`S3KnowledgeBaseDocBucket`].OutputValue' --output text)
